...
if (page.clientWidth > 569) header.style.backgroundPosition = `${headerTitle.clientWidth + padding}px bottom`;
else header.style.backgroundPosition = '0 86px';
...
function showSlides(n) {
    if (page.clientWidth < 569) {
        ...
        for(let i = 0; i < dots.length; i++)dots[i].classList.remove('our-story__dot--active');
...
function showFeatures(n) {
    if (page.clientWidth < 1024) {
        for(let i = 0; i < featuresCards.length; i++)featuresCards[i].style.display = 'none';
        if (n < 1) {
            ...
            slideIndex = 1;
        } else featuresCards[n - 1].style.display = 'unset';
    }
}
